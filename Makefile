...
DOCKER_REPO ?= hairyhenderson/ipupdater
DOCKER_TAG ?= latest
...
VERSION_FLAG := -X `go list ./version`.Version=$(VERSION)$(DIRTY)
# BUILD_DATE_FLAG := -X `go list ./version`.BuildDate=$(BUILD_DATE)

GIT_STATUS := `git status -s`
ifneq ("$(GIT_STATUS)","")
VERSION_FLAG := $(VERSION_FLAG)-dirty
endif
...
$(PREFIX)/bin/$(PKG_NAME)_%: $(shell find $(PREFIX) -type f -name '*.go')
	GOOS=$(shell echo $* | cut -f1 -d-) GOARCH=$(shell echo $* | cut -f2 -d- | cut -f1 -d.) CGO_ENABLED=0 \
		$(GO) build \
			-ldflags "-w -s $(BRANCH_FLAG) $(COMMIT_FLAG) $(VERSION_FLAG)" \
			-o $@ \
			./cmd/$(PKG_NAME)
